...
import { EmailResponse, Folder, LoginResponse, User, Users, SuccessResponse, MyFolderDetail, MyFolderDetail2, MyProjectFolder,MyProjectFolder2} from "../interfaces/Interfaces";
...
// ): Promise<AxiosResponse<MyProjectFolder, any> | null> => {
//     try {
//         const response = await possgAxios.post(
//             "community/folder",
//             { 
...
//         return response;
//     } catch (error) {
//         if(isAxiosError<MyProjectFolder>(error)) {
//             console.log(`Error: ${error.response?.status} ${error.message}`);
//             return null;
//         } else {
//             return null;
...
export const getMyFolder = async (
    ...
    try {
      ...
      console.log(`GET /community/folder -  Token: ${token}`);
      return response;
    } catch (error) {
      if (isAxiosError<MyProjectFolder>(error)) {
        console.log(`Error: ${error.response?.status} ${error.message}`);
